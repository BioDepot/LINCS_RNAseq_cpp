  ...
  if (mkdir $lockDir/lock$i 2> /dev/null ); then
   dir=${dirs[$i]}
		 ...
		 if ls $dir/all.fastq 1> /dev/null 2>&1; then
			echo "(bwa aln -l $BWA_ALN_SEED_LENGTH -t 1 $REF_SEQ_FILE $dir/all.fastq 2>/dev/null | bwa samse -n $BWA_SAM_MAX_ALIGNS_FOR_XA_TAG $REF_SEQ_FILE - $dir/all.fastq | $FILTERBIN > $dir/all.$suffix )" 		  
			(bwa aln -l $BWA_ALN_SEED_LENGTH -t 1 $REF_SEQ_FILE $dir/all.fastq 2>/dev/null | bwa samse -n $BWA_SAM_MAX_ALIGNS_FOR_XA_TAG $REF_SEQ_FILE - $dir/all.fastq | $FILTERBIN > $dir/all.$suffix ) 
		  rm $dir/all.fastq
		 else
