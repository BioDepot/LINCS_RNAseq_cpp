...
while [ "$IDX" -le "${LANES}" ]; do
	...
	SEQ_FILE="${SEQ_DIR}/${SAMPLE_ID}_${SUBSAMPLE_ID}_R2.fastq.gz"
	SEQ_FILES="${SEQ_FILES} ${SEQ_FILE}"
	let "IDX = $IDX + 1"
done	
...
echo "$UMITOOLS_DIR/source/w96/split_se -v -s 50 -o $ALIGN_DIR -t $THREAD_NUMBER  $SEQ_FILES"
$UMITOOLS_DIR/source/w96/split_se -v -s 150 -o $ALIGN_DIR -t $THREAD_NUMBER  $SEQ_FILES

echo "${UMITOOLS_DIR}/testScripts/multibwaNoMulti.sh $TOP_DIR $REF_DIR $SPECIES_DIR $ALIGN_DIR $BWA_ALN_SEED_LENGTH $BWA_SAM_MAX_ALIGNS_FOR_XA_TAG $THREAD_NUMBER $filterCmd"
$UMITOOLS_DIR/testScripts/multibwaNoMulti.sh $TOP_DIR $REF_DIR $SPECIES_DIR $ALIGN_DIR $BWA_ALN_SEED_LENGTH $BWA_SAM_MAX_ALIGNS_FOR_XA_TAG $THREAD_NUMBER "\"${filterCmd}\""
