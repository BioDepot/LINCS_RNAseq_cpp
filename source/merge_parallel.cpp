...
class Counts{
  ...
  //allocate memory for total
	spike_total=new unsigned int[erccListSize];
	spike_umi=new unsigned int[erccListSize];      
	spike_total_mm=new unsigned int[erccListSize];
    total=new unsigned int[geneListSize];
  ...
  for	(int wellIndex=0; wellIndex<NWELLS; wellIndex++){
   ...
   unordered_set<uint64_t>umi_seen_mm;
	...
	template <class T> void merge_filter(int nThreads,vector<string> &erccList ,vector<string> &geneList, vector<string> &wellList, string aligned_dir, umipanel<T> &barcodePanel,  unordered_map<string,string> refseq_to_gene,unordered_map<string,unsigned int>well_to_index,unordered_map<string,unsigned int>ercc_to_index,unordered_map<string,unsigned int>gene_to_index, uint32_t posMask, int binSize, int nbins, bool geneLevelFilter, bool mixtureOfWells){
		const uint64_t nCategories=geneList.size()+erccList.size()+1; //add1 for chrM
